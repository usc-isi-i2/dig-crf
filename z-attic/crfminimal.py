...
def input(year=2015, month=07, day=01, hour=01, location='hdfs', tag='incremental', fromPartNum=0, partNum=0):
    ...
    if location == 'hdfs':
        if tag == 'incremental':
            ...
            return ("/user/worker/process/istr58m/pilot01/ads_attrs_crfl/from-part-r-%05d-part-%05d.crf.out" % (int(fromPartNum), int(partNum)))
        else:
            raise RuntimeError("Unknown tag %r" % tag)
    elif location == 'local':
        return ("TBD/Users/philpot/Documents/project/dig-crf/data/input/%s/%04d-%02d-%02d-%02d-00/part-r-%05d"
...
def output(year=2015, month=07, day=01, hour=01, location='hdfs', tag='incremental', fromPartNum=0, partNum=0):
    if location == 'hdfs':
        if tag == 'incremental':
            ...
            return ("/user/worker/process/istr58m/pilot01/ads_attrs_crfa/from-part-r-%05d-part-%05d" % (int(fromPartNum), int(partNum)))
        else:
            raise RuntimeError("Unknown tag %r" % tag)
    elif location == 'local':
        if tag == 'incremental':
            # Let's mirror the HDFS convention for local as well
            return ("TBD/Users/philpot/Documents/project/dig-crf/data/output/incremental/pilot/refactor/ads_attrs_crfa/%04d-%02d-%02d-%02d-00"
                    % (year, month, day, hour))
        elif tag == 'istr58m':
            return ("TBD/Users/philpot/Documents/project/dig-crf/data/output/istr58m/pilot/ads_attrs_crfa/from-part-r-%05d"
                    % int(partNum))
        else:
            raise RuntimeError("Unknown tag %r" % tag)
    else:
