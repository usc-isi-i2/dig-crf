...
def isSequence(obj):
    ...
    return type(obj) in [unicode, list, tuple, buffer, xrange]
...
def positionIf(fn, seq, key=identity, fromEnd=False, default=None):
    i = 0
    for elt in reversed(seq) if fromEnd else seq:
        if fn(key(elt)):
            return len(seq)-i-1 if fromEnd else i
...
will raise ValueError for bad args"""
    ...
    if len(datestamps) == 0:
        ...
        for datestamp in datestamps:
            ...
            elif isinstance(datestamp, (int, long)):
...
def printHistogram(histogram):
    for key in sorted(histogram.keys(), reverse=True):
...
def coalesce(pairs):
    ...
    return set(tuple(s) for s in classes.values())
